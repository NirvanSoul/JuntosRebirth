-- Conserva los UUID generados por SQLite al materializar por primera vez una
-- entidad de Juntos. Así la restauración remota vuelve a encontrar el mismo
-- identificador local del creador, en vez de crear una segunda copia local.
-- Los identificadores antiguos no UUID siguen usando el UUID remoto generado
-- por PostgreSQL, por compatibilidad con instalaciones previas.
  ...
  for item in select * from jsonb_to_recordset(p_recurring_series) as x(
    ...
    if remote_series_id is null then
      insert into public.recurring_transaction_series(
        id, space_id, category_id, created_by, type, amount_minor, currency, title,
