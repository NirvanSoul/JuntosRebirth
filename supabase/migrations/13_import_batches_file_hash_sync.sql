-- Wires up `import_batches.file_hash`, prepared since migration 10 but never
-- populated by any client gateway (found in the import-feature audit,
-- 2026-08-09). The unique index on (user_id, space_id, file_hash) already
-- guards the common accidental double-tap; here the RPC additionally
-- degrades gracefully to file_hash = null on an actual collision so a user
-- who explicitly confirms "import again anyway" (client-side notice) never
-- gets stuck retrying a sync that can never succeed.
begin;
...
begin
  ...
  for batch_item in select * from jsonb_to_recordset(p_batches) as x(
    ...
    batch_file_hash := nullif(btrim(batch_item.file_hash), '');
    file_hash_conflict := false;
    if batch_file_hash is not null then
      select exists(
        select 1 from public.import_batches b
         where b.user_id = current_user_id
           and b.space_id = target_space_id
           and b.file_hash = batch_file_hash
           and b.id <> batch_item.id
      ) into file_hash_conflict;
    end if;
    ...
    insert into public.import_batches (
      ...
      case when file_hash_conflict then null else batch_file_hash end
      ...
      set source_type = excluded.source_type,
          ...
          file_hash = case
            when file_hash_conflict then import_batches.file_hash
