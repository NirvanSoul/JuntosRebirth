-- Las monedas de cada cuenta viajan en el mismo lote compartido.
--
-- `sync_couple_space_data` recibe ahora `balances` dentro de cada cuenta y las
-- reescribe enteras: una divisa retirada en el otro dispositivo tiene que
-- desaparecer también aquí, y calcular altas y bajas dejaría huérfana la que
-- ya no está.
...
begin
  ...
  for item in select * from jsonb_to_recordset(p_money_accounts) as x(
    ...
    -- Las monedas se reescriben enteras en vez de calcular altas y bajas: así
    -- una divisa retirada en el otro dispositivo desaparece también aquí.
    if jsonb_typeof(item.balances) = 'array' then
      ...
      select remote_money_account_id, p_space_id, balance.currency,
