...
-- ADR-080 Entrega 3: spaces.currency es inmutable tras la creación del espacio.
--
-- Cierra el bypass por RPC detectado en revisión: migrate_guest_data es
-- SECURITY DEFINER (corre como owner), así que la exención `current_user =
-- 'postgres'` del trigger le habría permitido mutar la moneda de espacios
-- existentes en nombre de cualquier usuario autenticado. La función pasa a ser
-- insert-only respecto a currency: la moneda solo se fija en el INSERT de
-- espacios nuevos (fallback 'EUR'); los espacios existentes solo actualizan
-- name y updated_at. El trigger bloquea además UPDATEs directos de roles no
-- privilegiados y preserva la exención para operaciones administrativas.
...
begin
  ...
  loop
    ...
    if remote_space_id is not null then
      -- Insert-only para currency (ADR-080): la moneda de un espacio existente
      -- nunca se muta desde este RPC, ni siquiera cuando el payload la trae.
...
as $$
begin
  -- El owner (postgres) puede modificar currency: operaciones de
  -- administración lo requieren. migrate_guest_data ya no muta currency
  -- (ver cabecera), así que esta exención no es alcanzable vía RPC.
...
drop trigger if exists spaces_currency_immutable on public.spaces;
