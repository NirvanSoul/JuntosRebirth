-- Las cuentas también viajan en la migración de invitado a cuenta de usuario:
-- si no, quien registró sus gastos con cuentas antes de registrarse perdería
-- esa clasificación al sincronizar por primera vez.
--
-- El lote sigue siendo idempotente por `p_batch_id`, y las cuentas se
-- procesan antes que series y movimientos porque ambos las referencian. Cada
-- una se resuelve por el par (instalación, id local), igual que la categoría.
...
-- La firma anterior se retira para que PostgREST no tenga dos candidatas.
drop function if exists public.migrate_guest_data(uuid, text, jsonb, jsonb, jsonb, jsonb);
